...
#ip route add default dev tun1 table vpn_net

# DEL route rule
#ip rule del from 10.8.0.0/24 table vpn_net
#ip route del default dev tun1 table vpn_net
...
#echo "150     vpn_net" >>/etc/iproute2/rt_tables
...
#!!!
#up /etc/openvpn/upstream-route.sh в конфиге client.conf script server 2
...
echo ""
echo "После ребута скачай конфиг указав ip адрес первого сервака"
echo "scp root@111.111.111.111:client000.ovpn vpn-double.ovpn"
